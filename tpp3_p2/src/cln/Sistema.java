...
public class Sistema implements Serializable{
	...
	public void agregaUsuario(String usuario, String contrasenia){
    ...
    * @param dni     DNI
	...
	public Pedido agregarPedido (int anio, int mes, int dia, String zona, boolean mascota, boolean usaBaul, int cantPasajeros, Cliente cliente) throws SolicitudIncoherenteException {
		...
		if (pedido.esValido() && this.hayExistenciaVehiculo(pedido.getCantPasajeros()))	{
			Pedidos.add(pedido);
			return pedido;
		}
		else
...
/**
 * Metodo para la validacion del pedido, verifica que exista por lo menos un solo vehiculo que cumpla con la cantidad de pasajeros solicitada.
 * @param cantPasajeros la cantidad de pasajeros solicitada en el pedido.
 * @return true si existe un vehiculo apropiado; false si no existe un vehivulo apropiado.
 */
	public boolean hayExistenciaVehiculo(int cantPasajeros) {
		boolean rta=false;
		Vehiculo v;
		int i=0;
		while(i<Vehiculos.size() && !rta){
			v= this.Vehiculos.get(i);
			if(v.getCantPasajeros()>=cantPasajeros)
			  rta=true;
			i++;
		}
		return rta;
	...
	public Viaje crearViaje(Pedido pedido, double distancia){
		ViajeFactory factoryviaje=new ViajeFactory();
		Viaje viaje = factoryviaje.getViaje(pedido, null, null, distancia,pedido.getZona());
 ...
 */
	...
	public ArrayList<Viaje> reporteViajesChofer(Chofer chofer, int anio0, int mes0, int dia0, int anio1, int mes1, int dia1){
		...
		for (Viaje viaje : Viajes){
			if (viaje.getChofer() == chofer){
				if ((viaje.getPedido().getFecha().compareTo(fecha0)>0) && (viaje.getPedido().getFecha().compareTo(fecha1)<0))
	...
	//		System.out.println("Imprime elementos del hashmap\n");
	//		String key=usuario.toString();
	//		String value=Usuarios.get(usuario).toString();
	//		System.out.println("Usuario: "+ key);
	//		System.out.println("Contrasena: "+ value);
	//	}
    //}
}
